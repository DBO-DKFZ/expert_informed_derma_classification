"""
Code from:
Repository: https://github.com/Schinkikami/PyT-ImbalancedDatasetSampler.git
Datei: imbalanced_sampler/ImbalancedDatasetSampler.py
Commit: 801fd9b (am 2022-01-20) 
"""
...
class ImbalancedDatasetSampler(Sampler):
	...
	def __init__(self, dataset: Dataset, sampling_factor: Union[str, float] = None, num_classes: Optional[int] = None , shuffle: bool = False, labels: Optional[Union[list, list[list]]] = None,
				 callback_get_label: Optional[Callable] = None, callback_type="single", class_label="hard", seed=42):
		...
		if len(labels) == len(dataset):
			...
			elif class_label == "soft":
				assert type(labels[0]) == list or type(labels[0]) == np.ndarray
				multi_label = {str(i):idx for idx, i in enumerate(np.unique(np.array([i for i in labels]), axis=0))}
				for i, label in enumerate(labels):
					self.labels[multi_label[str(label)]].append(i)
			else:
				raise NotImplementedError(
					"Implemented only cases for hard and soft label."
				)
		else:
			sum = 0
	...
	def __build_indices(self):
		...
		for c in range(self.num_classes):
			...
			random.seed(self.seed) # random.seed(config.seed)
			new_ind = (class_l * reps) + random.sample(class_l, sample)  # new_ind[-4:] -> [1471, 1532, 1418, 1433]
			indices += new_ind
